const BASE_URl_TEST = 'http://127.0.0.1:4523/m1/5708658-5389884-d3d6fe3f'
...
export const getRolesPage = async () => {
  try {
    const res = await fetch(BASE_URl_TEST + '/roles/page?pageNum=1&pageSize=5')
    return res.json()
  } catch (error) {
...
export const postRoles = async () => {
  ...
  const res = await fetch('http://127.0.0.1:4523/m1/5708658-5389884-d3d6fe3f/roles', requestOptions)
  console.log(res)
  if (res.ok) {
    return res.json()
